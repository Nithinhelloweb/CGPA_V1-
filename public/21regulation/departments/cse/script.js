...
window.onload = async () => {
  ...
  if (!batch) {
    showCustomAlert("Please select a batch to load the subjects.");
    return;
  }
  showLoader();
  try {
    ...
    showCustomAlert("Failed to load subjects for this page.");
  } finally {
    hideLoader();
  }
};
...
async function downloadFancyPDF() {
  if (!window.cgpaCalculated) {
    showCustomAlert("Please calculate your SGPA first before downloading the PDF.");
    return;
  }
  ...
  const username = document.getElementById("username").value.trim();
  if (!username || username.length !== 12) {
    showCustomAlert("Please enter a valid 12-digit register number.");
    return;
  }
  ...
  try {
    ...
    watermarkBase64 = await imageToBase64("/images/logo.png");
  } catch (err) {
